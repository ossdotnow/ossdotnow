...
function CommentContainer({ depth, isLast, hasReplies, children }: CommentContainerProps) {
    if (depth === 0) {
    ...
    return (
        <div className="relative" style={{ marginLeft: `${INDENT_SIZE}px` }}>
            ...
            >
                {/* Vertical line */}
                <div
                    className={`absolute left-0 top-0 w-0.5 ${isLast ? 'h-[24px]' : 'h-full'} bg-gray-50 opacity-10`}
                />
                {/* Horizontal connector - straight line without curve */}
                <div
                    className="absolute top-[22px] border-b-2 border-gray-50 opacity-10 h-0"
                    style={{
    ...
    return (
        <motion.div
            className={`relative ${depth === 0
                ? "rounded-none border border-neutral-800 bg-neutral-800/30 hover:bg-neutral-800/40 transition-colors"
                : "hover:bg-neutral-800/10 transition-colors duration-200"
                }`}
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.3 }}
        >
            <div className={`flex gap-3 relative ${depth === 0 ? 'p-3 sm:p-4' : 'py-2 px-3 sm:py-3 sm:px-4'}`}>
                <Avatar className="h-6 w-6 sm:h-7 sm:w-7 flex-shrink-0 mt-1">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className={`${depth === 0 ? 'mb-3' : 'mb-2'}`}>
                        <p className="leading-relaxed text-neutral-300 text-sm break-words whitespace-pre-wrap">
                            {comment.content}
                        </p>
                    </div>
